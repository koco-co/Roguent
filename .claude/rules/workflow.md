...
4. 实现、`bun run check`、`bun test`、`bunx tsc --noEmit`(`check` 只 Biome 不查类型,改 TS 必跑此条)、按主题分批 commit,全部在 worktree 内完成。
...
- body 可中文。
- 改后即测:动了代码 / 配置 / runtime 就跑 `bun test` + `bun run check` + `bunx tsc --noEmit`(`check` 不查类型,改 TS 必跑后者),失败先修;不把局部通过说成全量通过。
