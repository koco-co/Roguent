...
- 2026-06-12:**slash 菜单 / 法术书补全 SDK `skills`**(fix `d54c568`)。根因:SDK `system:init` 把可用命令分 `slash_commands`(有命令文件的命令)与 `skills`(superpowers 等迁成纯 skill 的命令,如 /brainstorming /writing-plans)两字段上报,`normalize.ts` 只取前者、整列丢弃后者,前端只见寥寥几条而非交互式 CLI 的一长串。沿 slashCommands 同构补 `skills` 全链路(events/domain/normalize/session/codex-normalize/transcript/store + Composer `/` 菜单 + Skills 面板,均按名去重)。点击发 `/${上报名}`(bare 名与 plugin:name CLI 都认,不造假命令);CLI 内建命令(/add-dir /agents 等)是 REPL 专属、SDK 从不上报,不纳入。门禁 744 单测 + tsc + check 全绿。
