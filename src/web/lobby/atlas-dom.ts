...
let cacheJsonUrl: string | null = null;
let promise: Promise<AtlasDom> | null = null;
let promiseJsonUrl: string | null = null;

function load(): Promise<AtlasDom> {
  const urls = resolveCurrentArtPackAtlasUrls();
  if (cache && cacheJsonUrl === urls.json) return Promise.resolve(cache);
  if (!promise || promiseJsonUrl !== urls.json) {
    promiseJsonUrl = urls.json;
    promise = (async () => {
...
export function useAtlasDom(): AtlasDom | null {
  ...
  useEffect(() => {
    ...
    refresh();
    window.addEventListener(ARTPACK_CHANGE_EVENT, refresh);
    return () => {
      cancelled = true;
      window.removeEventListener(ARTPACK_CHANGE_EVENT, refresh);
    };
  }, []);
  return data;
}
