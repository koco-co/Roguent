...
(function(){
    ...
    return h('div',{className:'scrim',onClick:onClose},
      h('div',{className:'panel rivets modal-pop'+(vibe?' vibe-'+vibe:''),style:{width,maxHeight:'960px',height},onClick:e=>e.stopPropagation()},[
        h('div',{key:'tb',className:'panel-titlebar'},[
          ...
          sub&&h('span',{key:'s',className:'sub cjk'},T(sub)),
    ...
    return h(Modal,{title:'NPC',sub:'会话档案',icon:'account',accent:statusColor,onClose,width:760},
      h('div',{className:'npccard'},[
        ...
        npc.status==='askuser'&&h('div',{key:'ask',className:'ask-banner askpulse'},[
          ...
          h('div',{key:'t'},[h('div',{key:'1',className:'px',style:{fontSize:10,color:'#36c5e0',marginBottom:4}},T('待你回应')),h('div',{key:'2'},npc.ask)]),
        ]),
        h('div',{key:'stats',className:'statgrid'},[
          row(T('项目'),DATA.room.project),
          row(T('模型'),npc.model,' gold'),
          row(T('模式'),npc.orchestrator?'orchestrator':'subagent'),
          row(T('状态'),statusText(npc.status)),
          npc.orchestrator&&row(T('子智能体'),Object.entries(npc.sub).map(([k,v])=>statusText(k)+' '+v).join(' · ')),
          row('Token',npc.tokens.toLocaleString()),
          row(T('花费'),'$'+npc.cost.toFixed(2)),
        ]),
        // context bar
        h('div',{key:'ctx',className:'npccard-ctx'},[
          h('div',{key:'l',className:'sr-label'},['Context ',h('span',{key:'p',className:'px',style:{fontSize:9}},npc.util+'%')]),
        ...
        h('div',{key:'comp',className:'comp-box'},[
          h('div',{key:'h',className:'comp-h'},[
            ...
            h('span',{key:'t',className:'px',style:{fontSize:10}},window.TL('Context 压缩阈值','Context threshold')),
          ...
          h('div',{key:'seg',className:'seg'},[
            h('div',{key:'i',className:'seg-opt'+(mode==='inherit'?' on':''),onClick:()=>setMode('inherit')},T('继承全局 (20%)')),
            h('div',{key:'o',className:'seg-opt'+(mode==='override'?' on':''),onClick:()=>setMode('override')},T('局部覆盖')),
          ]),
          ...
          h('div',{key:'note',className:'faint',style:{fontSize:11,marginTop:6}},mode==='inherit'?T('跟随设置面板的全局默认（Opus 20%）。'):T('此 NPC 单独生效，优先级高于全局。')),
        ]),
        // actions
        h('div',{key:'act',className:'npccard-act'},[
          h('button',{key:'enter',className:'pxbtn primary cjk',onClick:onClose},T('进入')),
          h('button',{key:'chat',className:'pxbtn cjk',onClick:()=>onOpen('chat')},T('聊天')),
          h('button',{key:'arch',className:'pxbtn cjk'},T('归档')),
          confirmDel
            ? h('button',{key:'del',className:'pxbtn danger cjk',onClick:onClose},T('确认删除？'))
            : h('button',{key:'del',className:'pxbtn cjk',style:{color:'#ff8197'},onClick:()=>setConfirmDel(true)},T('删除')),
        ]),
      ])
    );
  }
  function statusText(s){return T({working:'工作中',thinking:'思考',askuser:'待回应',todo:'待办',idle:'待命',done:'完成',error:'出错',compacting:'压缩中'}[s]||s);}
  ...
  function Tasks({onClose}){
    ...
    const npcName=id=>{const n=DATA.room.npcs.find(x=>x.id===id);return n?n.name:T('待认领');};
    ...
    const stateMeta={pending:[T('待领'),'#8a8170'],'in-progress':[T('进行中'),'#36c5e0'],completed:[T('完成'),'#5fd35f']};
    ...
    return h(Modal,{title:'TASKS',sub:'共享任务清单 · agent teams',icon:'quest',onClose,width:1180},
      h('div',{className:'tasks-wrap'},[
        ...
        askers.length>0&&h('div',{key:'ask',className:'task-askbar'},[
          ...
          h('span',{key:'t',className:'px',style:{fontSize:10,color:'#36c5e0'}},T('待你回应')+' ('+askers.length+')'),
        ...
        h('div',{key:'cols',className:'tasks-cols'},[
            ...
            ...byState[st].map(t=>{
              ...
              return h('div',{key:t.id,className:'task-item'+(sel===t.id?' sel':''),onClick:()=>setSel(t.id)},[
                ...
                h('div',{key:'mt',className:'task-sub'},[
                  t.owner?h('span',{key:'o',className:'task-owner'},[heroOf(t.owner)&&h(PixelSprite,{key:'p',base:heroOf(t.owner),anim:'idle',scale:1.6}),npcName(t.owner)]):h('span',{key:'o',className:'faint'},T('待认领')),
                  blk&&h('span',{key:'b',className:'chip',style:{color:'#ff8197'}},T('阻塞中')),
                  t.blockedByUser&&h('span',{key:'u',className:'chip askpulse',style:{color:'#36c5e0'}},T('等用户')),
                ]),
              ]);
            }),
          ]))),
          ...
          h('div',{key:'r',className:'task-detail'},cur?[
            ...
            h('div',{key:'meta',className:'task-d-meta'},[
              metaLine(T('归属'),cur.owner?npcName(cur.owner):T('待认领')),
              metaLine(T('模型'),cur.model),
              metaLine(T('依赖'),cur.deps.length?cur.deps.map(d=>{const dep=DATA.tasks.find(x=>x.id===d);return (dep?dep.title:d)+(dep&&dep.state==='completed'?' ✓':' ⧖');}).join('，'):T('无')),
            ]),
            h('div',{key:'tl',className:'task-timeline'},[
              h('div',{key:'h',className:'px',style:{fontSize:9,color:'#8a8170',marginBottom:8}},T('状态时间线')),
              h('div',{key:'1',className:'tl-step done'},window.TL('创建 · 19:02','Created · 19:02')),
              cur.state!=='pending'&&h('div',{key:'2',className:'tl-step done'},window.TL('认领 · 19:05','Claimed · 19:05')),
              cur.state==='completed'?h('div',{key:'3',className:'tl-step done'},window.TL('完成 · 19:41','Done · 19:41')):h('div',{key:'3',className:'tl-step now'},window.TL('进行中…','In progress…')),
            ]),
            cur.state==='pending'&&!cur.blockedByUser&&h('button',{key:'claim',className:'pxbtn primary cjk',style:{marginTop:14}},T('认领任务')),
          ]:h('div',{className:'faint'},T('选择一个任务'))),
        ]),
        // mailbox
        h('div',{key:'mb',className:'mailbox'},[
          h('div',{key:'h',className:'px',style:{fontSize:10,color:'#f2c84b',marginBottom:8}},[h(Icon,{key:'i',name:'chat',size:16,style:{marginRight:6,verticalAlign:'middle'}}),T('信箱 · inter-agent')]),
  ...
  function Leaderboard({onClose}){
    ...
    return h(Modal,{title:'LEADERBOARD',sub:'按 token 降序',icon:'trophy',onClose,width:1080},
      h('div',{className:'lb-wrap'},[
        h('div',{key:'tabs',className:'tabs'},[['session',T('按会话')],['model',T('按模型')],['runtime',T('按 runtime')]].map(([k,l])=>
